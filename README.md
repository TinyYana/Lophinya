...
| **不是效能 fork** | 效能全部來自 Folia 跟 Lophine，這一層加的只有相容性，而相容性通常是拿效能去換的 |
...
| **方塊讀取** | 跨 region／跨世界的唯讀查詢改由已在記憶體的區塊回答；不在記憶體裡，一般情況照舊大聲失敗——因為那需要載入，是另一回事。但呼叫端根本不擁有任何 region 時（啟動期、或全域執行緒，構造上本來就不會跟任何 region 搶資料）改成直接把它載進來再讀，讓新世界／首次安裝這種一定會撞到空區塊的情境不用整個失敗 |
...
| 項目 | 內容 |
|---|---|
| **上游同步** | 跟過一次完整的 rebase 演練，自己的 patch 全部乾淨重播、0 個 reject。唯一的衝突是 `README.md` 自己（上游改了一行空白，這邊整份重寫過），純文字衝突，跟程式碼無關 |
| **生電行為對照 Paper** | 同一支探針、同一份世界，兩個平台各測兩次。活塞推動上限（含 12 格上限那個邊界情況）、TNT 引信 tick 數、紅石中繼器時序、漏斗流速，兩邊數字逐位元組相同 |
| **資料遷移＋回退到 Paper** | 帶著既有資料寫入、重啟讀回、再換成 Paper 的 build 讀一次——三次冷啟動全部乾淨（沒有錯誤或警告等級的例外），寫進去的標記值三個檢查點都讀得回來 |
| **全量插件同場冷啟動** | 一份接近正式服規模的插件集合（40＋顆）連續冷啟動兩次，結果逐項相同，沒有未解釋的 thread ownership 例外 |

殘留的已知缺口：一項跟飛行判定有關的反作弊行為還沒有真人在正式服規模下覆測過，
資料遷移目前只有一種資料格式端到端證明過。這兩項還沒有就沒有，不會因為其他項目
過了就含糊帶過
